...
6. **Autonomous ≠ ungoverned.** A run's autonomy mode only changes *who answers* the permission question — a waiting human (supervised) vs the policy's pre-decided fallback (autonomous) — never *whether it is asked*. Autonomy is never the reason a call skips the gateway. What *routes* a call to the gateway is provider-shaped, and the two halves differ in strength: brokered MCP calls reach it structurally (the control plane executes them), while in-sandbox calls reach it because the runner routes them there — on the Claude harness by a mandatory `PreToolUse` hook, because the SDK's permission *callback* alone is an ask-path handler and whole tool classes were auto-approved beneath it. The callback stays wired in both modes (never the SDK's `bypassPermissions`) **and** the hook makes it mandatory; `scripts/gate-proof.sh` proves the composition at zero model cost. Containment is the layer that binds a harness or pinned image that does not route. As autonomy rises, budgets and containment tighten — they never loosen. Long-running agents are first-class citizens of the same lifecycle, not a separate system.
...
| Workspace init | **A formal post-startup init phase** (`initializing`, §6.7): container/VM starts → orchestrator materializes the workspace through provider APIs → agent starts. The credentialed fetch stays **control-plane-side** — an in-container token would be readable by the agent moments later, and MicroVM network connectors are fixed at run time, so "egress only during init" can't exist on the flagship runtime. The sandbox is egress-free BY DEFAULT — and since the governed-network-access epic, any wider reach is an explicit `NetworkGrant` frozen into the RunSpec and enforced at L3/L4, never an ambient capability. (This cell previously read "the sandbox stays egress-free always", which was already inaccurate for the Docker `host-dev` profile and is now superseded.) |
