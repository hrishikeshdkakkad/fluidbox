...
**Date:** 2026-07-10 (rev 5: borrow-the-agent Phase 3 shipped) · **State:** M0 + M1 + §6.A hardening + **connected git workspaces (Phase 1)** + **generic API borrowing & signed callbacks (Phase 2)** + **scheduled borrowing (design-doc Phase 3)** done; `just e2e` = 175 checks green across 6 phases.
...
- **Quality bar:** `cargo clippy --workspace -D warnings` clean; 65 unit/integration tests green; **`just e2e` = 175 acceptance checks green** (9 live demo A + 14 governance + 26 git workspaces + 50 api triggers + 54 scheduled borrowing + 22 failure-path). ~10,000 lines of Rust across 5 crates.
