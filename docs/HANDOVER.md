...
**Date:** 2026-07-10 (rev 4: borrow-the-agent Phase 2 shipped) · **State:** M0 + M1 + §6.A hardening + **connected git workspaces (Phase 1)** + **generic API borrowing & signed callbacks (design-doc Phase 2)** done; `just e2e` = 121 checks green across 5 phases.
...
- **Quality bar:** `cargo clippy --workspace -D warnings` clean; 57 unit/integration tests green; **`just e2e` = 121 acceptance checks green** (9 live demo A + 14 governance + 26 git workspaces + 50 api triggers + 22 failure-path). ~9,000 lines of Rust across 5 crates.
...
- **Run the acceptance suite:** `just e2e` (stop `just dev` first — the suite owns the stack). Phases: live demo A (self-skips without `ANTHROPIC_API_KEY`), governance plane, git workspaces, api triggers + signed callbacks, failure paths. Unit tests: `set -a; source .env; set +a; cargo test --workspace`.
...
- **Phase 3 (scheduled borrowing)** — ⏭ **next**. Design intent agreed 2026-07-10: a schedule is **not a new object** — it's a trigger subscription with a clock (`InvocationContext.kind=schedule`); every firing goes through the same `run_service::create_run`, and signed-callback delivery already exists. New machinery is deliberately small: a `schedules` table (§10 seed: cron, **explicit timezone** for DST-correctness, `next_fire_at`, `missed_run_policy`, `last_fired_at`), `concurrency_policy` finally landing on `trigger_subscriptions` (deferred from Phase 2 so config never lies), and a tick worker shaped like `deliveries.rs`. **Exactly-once firing reuses the Phase 2 idempotency claim table with a deterministic key (subscription + scheduled fire time)** — a crashed or double-fired scheduler cannot duplicate runs. The two time-specific policies: overlap (`allow` | `skip_if_running` | `replace`) and missed-run (skip vs one catch-up firing; "fire all missed" is a thundering herd — avoid). Governance matters more here: schedules run unwatched (usually autonomous) and costs recur, so policy/budget ceilings are the safety net. E2E needs fire-fast testability (sub-minute cron or an equivalent test seam). **§17 decision to settle at the boundary: #5** — planned recommendation: default `skip_if_running` + skip missed (classic cron semantics), catch-up and `allow` as opt-ins, `replace` for latest-state jobs. Acceptance (§12): a repository-maintenance agent runs on schedule, skips or serializes overlapping work per policy, and publishes its result.
