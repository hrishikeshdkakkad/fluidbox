...
fluidbox does not try to prevent sandbox compromise — it **assumes** it. Prompt injection means the agent itself must be modeled as adversarial, and the agent executes arbitrary code by design. The security model therefore never depends on sandbox good behavior for the things that are *structurally* unreachable from inside: credentials, upstream URLs, and — on the enforcing profiles — general egress.

**The decision gate is the one place where "never depends on sandbox good behavior" needs qualifying, and this document previously overstated it.** The earlier wording — *"everything the sandbox can do passes the control plane's single decision gate"* — cannot be true of a workload that, by T2's own definition, executes arbitrary code. The accurate split:

- **Brokered MCP tools: structural.** The control plane executes the call. Nothing the sandbox does produces the effect, so the gate binds T1 **and** T2.
- **In-sandbox tools** (`Bash`, `Edit`, `Read`, sandbox stdio MCP): the sandbox executes them, and the gate binds them because the runner **routes** every call to it. On the Claude harness that routing is a mandatory `PreToolUse` hook — upstream's own documented remedy for the fact that the SDK's permission callback is an ask-path handler only. It is enforced, and it is proven end-to-end with real side effects and at zero model cost by `scripts/gate-proof.sh`, which runs as a CI job. Against **T1** (a prompt-injected model emitting tool intents through the harness) that is a real control. Against **T2** (a workload already executing arbitrary code, which is not obliged to route anything) it is **not** a control — and an older pinned `runner_image` routes nothing, with no server-side detection today.

So for in-sandbox tools, **containment** — the sandbox network mode and the disposable workspace — is the layer that binds T2, and the gate provides intent-time policy plus audit for a cooperating harness. Capability ≠ permission ≠ containment remains the right frame, and this is precisely why the third layer is not optional on the Docker default profile, where it is absent.
...
| Bypass the permission callback (autonomy modes) | The callback stays wired in both modes — never the SDK's `bypassPermissions`; autonomous mode rewrites `RequireApproval` to the policy fallback inside `evaluate()`, ledgering both verdicts | shipped — but this row answered the wrong question until 2026-07-30. *Is the callback wired?* was shipped all along; the attack is *is it invoked?* It was not: the SDK translates `canUseTool` into the CLI's `--permission-prompt-tool`, which the CLI consults only for calls it decides to **ask** about, so its read-only/safe-command classifications executed with the callback never running. Closed by a mandatory unscoped `PreToolUse` hook that forces every call back onto the callback, plus a `GateWitness` tripwire that aborts the run if a `tool_result` arrives for a call no decision was made for. Proven by `scripts/gate-proof.sh` (CI, no model spend): on the unfixed image a read-only-classified `Bash` command executed with **zero** `/permission` calls; on the fixed image the same command is refused and leaves no side effect, with allow-path positive controls |
| Skip the gate by pinning an OLD runner image, or by using a harness that does not route | **Not controlled.** Mediation is runner-side by construction — the control plane cannot intercept in-sandbox execution — so an image that never asks is not detected: there is still no server-side check for a terminal run with a non-empty diff and zero `tool.decision` events. Containment is the binding control here, and the Docker default profile (`HostDev`) does not provide it | **not shipped** — tracked as the highest-value remaining detection item |
| Escape the gate through nested sub-execution (`Agent`, `Task`, `Workflow`, `Skill`, `TaskCreate`) | Nested tool calls may not surface as top-level `tool_use`/`tool_result` blocks, so they could be neither routed by the hook nor seen by the tripwire — the tripwire documents itself as a knowingly incomplete detector for exactly this. **The seed policy therefore DENIES all five** rather than asking a human to authorise an unobserved tool tree (`policies/default.yaml`; pinned by `seed_policy_governs_the_advertised_surface`) | shipped as a **policy** control; the underlying routing question is untested and is the most important open item on the Claude harness |
...
| Insufficient-scope challenge used to phish scope escalation | An SEP-835 `insufficient_scope` challenge on a 401/403 is terminal for the call — no re-mint, no retry — plus token eviction. It marks the connection `status='error'` with `insufficient_scope: reconnect with more scopes` (the server's requested scopes recorded in the note only) **only when the credential was an OAuth one**; create_run, photograph, and the broker all already fail closed off connection status. The broker never auto-escalates a frozen grant | shipped (Phase E) |
| Insufficient-scope challenge used as a remote kill switch on an org's connection | The marking half is narrowed to OAuth connections (`broker.rs::insufficient_scope_target` returns the connection id only when `BrokeredAuth.oauth_connection` is `Some`). A hostile server picks its own `WWW-Authenticate` header, so an unconditional marking would let any upstream disable an org's integration with one header — and for a STATIC credential (API key / custom header / Basic) there is no scope grant to widen, so "reconnect with more scopes" is not an actionable remedy anyway. The call is still terminal on every path, so the design's no-auto-escalation property is unchanged; only the blast radius shrinks. The status write is additionally guarded by an `authorization_generation` CAS, so a challenge from a superseded generation cannot mark a reconnected connection | shipped (Phase E) |
...
| **Same-uid `ptrace` of the runner-control token** (Phase F narrowed this) | **The `/proc/<pid>/environ` read is CLOSED.** Both images now `ENTRYPOINT` through a shared POSIX-sh script that writes the token to a 0600 `mktemp` file, opens it as fd 3, **unlinks** it, unsets the variable, and `exec`s the runner — `execve` installs a brand-new environ region on the same pid, so node stays pid 1 while its `/proc` environ never contains the credential, and the runner closes the descriptor before spawning anything (an inherited fd carries no `FD_CLOEXEC`). **What remains is `ptrace(2)`**: a same-uid child can still attach to the runner and read the token from live memory, and neither `capabilities: drop ALL`, nor `allowPrivilegeEscalation: false`, nor `seccompProfile: RuntimeDefault` blocks same-uid ptrace. Only a uid split or a separate container (its own PID namespace) closes that. The env fallback — entrypoint bypassed by a direct `node index.mjs` or an overriding manifest — re-opens the original residual in full; it is deliberate compatibility, and a test asserts it so its cost stays visible. Docker's `HostDev` network mode is explicitly not a boundary at all |
...
| **An upstream MCP session we cannot authenticate is never `DELETE`d** (Phase F closed the replica-local leak) | The replica-local leak is **closed**: migration 0024 records one durable row per `(run session, peer, owning replica)`, and terminal cleanup now tears down the local map **union** the durable rows, so whichever replica finalizes the run closes every replica's sessions — and because the rows are durable, that cleanup is now retryable rather than one-shot. Replicas deliberately do **not** adopt each other's upstream sessions (that would put two replicas on one session with no serialization, to save an `initialize` MCP allows us to repeat). **What remains:** the `DELETE`'s authorization is re-resolved live and never stored, so when the credential is unresolvable — a revoked connection, the commonest case reaching the sweeper — invariant 9 forbids sending one at all. Those rows are retired without an upstream `DELETE` after a grace period, and the session expires on the upstream's own schedule. That is a session slot, not a correctness or custody problem, and it is strictly better than Phase E, where it leaked until process exit |
...
| **An FQDN network grant enforces as the RESOLVED ADDRESS SET, not as a per-connection binding to a DNS answer** | Proven directly on Cilium 1.19.6 (Phase 0 spike): a granted name's resolved addresses become reachable by raw IP, cluster-wide within the DNS cache TTL, by every endpoint the policy selects — including a pod that never performed the lookup, because `toFQDNs` resolution produces cluster-wide CIDR identities. A run granted `pypi.org` can therefore reach `pypi.org`'s current addresses directly. It **cannot** reach anything else (verified: an ungranted external target stayed denied throughout), so the grant is still a containment boundary — but its unit is "the addresses these names resolve to", and the DNS-rebinding window is the same fact seen from the other side. |
| **Cross-run network isolation depends on the per-run policy's selector** | The per-run `CiliumNetworkPolicy` selects on all three run identity labels (session, tenant, run), and that is what creates the isolation: with a shared selector, the finding above pools every concurrent run's granted addresses into one reachable set. Verified both ways on kind — two runs with different grants could reach neither each other's FQDN nor its raw IP. A unit test pins the selector at exactly three labels, so widening it fails CI rather than silently removing isolation. |
| **Attribution under a shared egress gateway is by flow, not by source IP** | Every run behind one egress-gateway node shares its SNAT address, so "which run made this connection" is answered from Hubble flows (and the per-session policy identity), never from the observed source address. The gateway is SNAT plus attribution only and is never the security boundary — the L3/L4 policy is, and it holds whether or not the gateway is in the path. |
| **Egress-gateway startup slip** | A pod may be scheduled and policy-enforced before the egress-gateway datapath has converged for it, so its first connections can egress with the node's own address rather than the gateway's. This affects stable-egress-IP expectations and attribution, NOT containment: what may be reached is decided by the policy either way. |
| **A `public` grant is an exfiltration channel by construction** | `public` means "everything the deny wall permits", so a run with it can post data anywhere on the internet. That is the point of the mode, not a defect — the mitigations are that it is refused for a run holding brokered tool results unless policy opts in (credentials plus reach is the dangerous pairing), it requires an explicit policy ceiling and an agent declaration, it can require human authorization, and the deny wall still forbids metadata, cluster ranges, and the deployment's own public addresses. |
