...
| LLM quota | `shared` gateway key or per-tenant LiteLLM virtual keys (`FLUIDBOX_LLM_KEY_MODE`, master key confined to provisioning — shipped); per-run budget still checks-then-records | Durable per-run budget reservations that remove the concurrent-call race | D (quota, shipped); E (reservation race, Gap 14) |
