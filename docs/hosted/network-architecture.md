...
This is the hosted network diagram: every plane, every listener, and every edge — who initiates it, how it authenticates, what crosses it, and what must never cross it. Edges are annotated with the phase that lands them where they are not shipped today — browser sessions/PATs are Phase B, run resource bindings are Phase C — and every current-vs-target difference is listed explicitly in [Target-state deltas](#target-state-deltas), never implied.
...
|---|---|---|---|---|---|
| 1 | Browser → ingress `/` (dashboard) — sso mode **Phase B** | Browser | `__Host-fbx_web` session cookie (sso mode) | UI assets; proxied API calls with forwarded cookies, CSRF header, normalized `Origin`, and propagated `Set-Cookie` | The admin token — in `sso` mode it is not present in the web app's environment at all; a missing cookie fails 401, never falls back to operator authority. (Today: static `admin` mode — the proxy injects the admin token server-side.) |
| 2 | Browser → `/v1` (same origin, via proxy) — **Phase B** | Browser | Session cookie; CSRF custom header + `Origin` check on every non-GET; dual-credential requests (cookie **and** bearer) rejected | API requests/responses, SSE timelines (re-authorized on a ≤60 s interval) | Browser-supplied `tenant_id`/`user_id` as trusted fields — principals derive solely from verified sessions |
| 3 | CLI/API → `/v1` | Client | `Bearer fbx_pat_…` (**Phase B**; membership rechecked per use); trigger tokens for exactly their one subscription (shipped); admin token — confined to `/v1/admin/*` when `FLUIDBOX_REQUIRE_SSO=1` (**Phase B**) | API requests | CSRF headers (bearer clients are exempt — not ambient authority) |
...
| 5 | Browser ↔ per-org IdP (login) — **Phase B** | Browser | The IdP's own session/consent | OIDC authorization redirects (code + sealed `state`; `__Host-fbx_login_{flow}` cookie binds the initiating browser) | fluidbox credentials of any kind |
| 6 | Control plane → per-org IdP — **Phase B** | Control plane | Sealed client secret (or PKCE-only public client) at the token endpoint | Discovery documents, JWKS, code exchange — all SSRF-validated (https, re-validated redirects, forbidden address classes), and never inside a DB transaction | `offline_access` requests; refresh tokens are discarded in v1 |
...
| 8 | Control plane → connector authorization servers | Control plane | Per-connection client identity (pre-registered sealed secret → CIMD → DCR) with PKCE S256 and RFC 8707 `resource=` on both legs | RFC 9728 protected-resource metadata, RFC 8414/OIDC discovery, token exchange, refresh-token rotation | Credentials toward hosts outside the admitted base (audience binding); the full SSRF boundary for these fetches completes in **Phase E** (Gap 7) |
...
|---|---|---|---|
| Browser authentication | Static `admin` proxy mode: the dashboard proxy injects the operator admin token server-side; no users, sessions, or PATs exist (Gaps 1–2) | `sso` mode cookie passthrough; `__Host-fbx_web` sessions, PATs, CSRF/`Origin` enforcement, per-org OIDC (edges 1–3, 5–6) | B |
| Run resource bindings | Brokered capability pins embed a concrete connection; the orchestrator resolves workspace/publish credentials directly (Gap 3) | Typed `mcp`/`workspace_fetch`/`result_publish` bindings; orchestrator, broker, and delivery worker consume binding IDs (edge 17) | C |
