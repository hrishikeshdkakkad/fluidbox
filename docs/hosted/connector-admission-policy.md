...
- **Rate limits (shipped, Phase E; a fourth dimension and a durable tier in Phase F):** *independent* token buckets — per tenant, per connection, per upstream host — checked before each brokered dial. All are peeked before any is charged, so a refusal costs nothing; `0` on a dimension disables that dimension (it never means "block everything"). Knobs: `FLUIDBOX_EGRESS_RATE_{TENANT,CONNECTION,HOST}_PER_MIN` (defaults 120 / 60 / 120).
- **Per-user limiting (shipped, Phase F):** `FLUIDBOX_EGRESS_RATE_USER_PER_MIN`, default **60, on by default**, keyed on the run's invoking user. It closes the hole the tenant bucket could not see: one member of an org spreading dials across the org's connections and consuming the whole tenant budget. Two things an operator must know before upgrading. **(1) It aggregates.** 60/min is that user's entire outbound budget across *all* of their concurrent runs and connections — not 60 per run. A user running with the design's assumed working set of 3 attached MCP servers was effectively bounded at 3 × 60 = 180/min by the per-connection ceiling; under this dimension they are bounded at 60/min in total. That is a deliberate behaviour change for fan-out shapes, not a no-op default. **(2) It is durable-tier only** — the in-memory governor is unchanged, so `FLUIDBOX_EGRESS_DURABLE=0` disables per-user limiting entirely. A run with no invoking user (trigger and schedule invocations) skips the dimension rather than sharing one bucket for all automation.
- **A refusal charges nothing, on any dimension.** The in-memory tier peeks every dimension before consuming a token; the durable tier increments and then rolls the whole transaction back on a refusal. Both matter for the same reason: refusal precedence reports `tenant` first, so charging a refusal raised by the narrower `connection`/`user`/`host` tiers would push the org's shared bucket over and throttle every other connection and user in the organisation for the rest of the minute — a self-inflicted denial of service triggered by one runaway connection or one sick upstream.
- **Circuit breakers (shipped, Phase E; durable in Phase F):** keyed `(tenant, connection, upstream host)` — strictly finer than per-connection. They count *transport* failures only (connect refused, timeout, mid-stream read failure, refused redirect, inadmissible URL, HTTP 5xx); a JSON-RPC error, a 4xx, a 401/SEP-835 challenge, or a tool result that merely reports `isError` all mean the upstream is **healthy** and never trip it. Open for `FLUIDBOX_EGRESS_BREAKER_OPEN_SECS` (default 60) after `FLUIDBOX_EGRESS_BREAKER_THRESHOLD` (default 5) consecutive failures, then one half-open probe carrying a deadline so a lost probe cannot wedge it shut. The durable breaker's probe election is a single conditional UPDATE carrying an epoch: the guarantee is that at most one probe can *decide*, not that only one is ever in flight — a full dial can outlive the open window, so overlapping probes are routine and every superseded epoch is inert in both directions.
- **Cross-replica enforcement (shipped, Phase F; `FLUIDBOX_EGRESS_DURABLE`, default on).** Both controls are still evaluated in-memory per replica *first* — free, and it stops a runaway loop with no database load — and a dial must then also pass a Postgres-backed tier (migration 0023) that makes the numbers deployment-wide. With the durable tier off, the older disclosure applies unchanged: N replicas ⇒ N× the configured ceiling, and a breaker opened on one replica does not stop the others. Three properties are worth stating:
  - **It costs.** Roughly 9 extra database round trips per brokered dial, and every dial belonging to one tenant serialises on that tenant's single rate-window row.
  - **It degrades, never fails.** A database error admits on the local verdict alone and increments a counter; this is a fairness/abuse control, not a quota system, and refusing every brokered call because the bookkeeping store is unreachable would amplify one outage into two.
  - **A permanently unusable tier is reported, once, loudly.** Because degrading is silent by design, the server runs a boot usability probe and logs at ERROR with the remediation if it cannot read and write the governance tables as the role it runs as — the realistic cause being migration 0023's DML grants skipped because the runtime role did not exist when it ran. Without that probe a permanent misconfiguration is indistinguishable from a transient blip, and the deployment believes it has a cross-replica ceiling it does not have.
  - One dimension stays local-only and per-replica: the deliberately loose cross-tenant `host_global` stampede ceiling (8× the per-tenant host ceiling). Keying it durably would require a per-dial RLS bypass, which is a worse trade than the disclosed looseness.
- A refusal from any of these lands as a **`failed_before_send` execution claim** (positively never sent, therefore re-claimable) and surfaces to the model as a retryable tool error naming the scope and a retry delay, with the host as a `sha256:` digest — never the raw host.
