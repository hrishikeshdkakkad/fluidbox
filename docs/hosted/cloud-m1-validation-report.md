...
| 4 | invited owner logs in through the Vercel origin | **CONFIGURED AND VERIFIED UP TO CONSENT** — `FLUIDBOX_PUBLIC_URL` is the Vercel origin, so the authorize leg now carries `redirect_uri=https://fluidbox-cloud-dashboard.vercel.app/v1/auth/callback` with nonce + PKCE S256 + state (the callback-host trap avoided). Completing the leg needs the IdP's real client secret and a human at the consent screen | live `/v1/auth/login/fluidzero/start` |
