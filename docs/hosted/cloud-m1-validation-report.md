...
| 4 | invited owner logs in through the Vercel origin | **BLOCKED — needs a standards-conformant IdP; escalated as an M3 core proposal.** The deployment side is fully configured and correct: `FLUIDBOX_PUBLIC_URL` is the Vercel origin and the authorize leg carries `redirect_uri=…vercel.app/v1/auth/callback` with nonce + PKCE S256 + state. Attempting the real login proved **WorkOS's `/user_management/authorize` is not standards-conformant** — it rejects a compliant OIDC request with `invalid-connection-selector` and only works with the proprietary `provider=authkit`. Closing this needs one BYO-IdP's issuer/client/secret; no core change and no further platform work | readiness ledger, "DECISIVE IDENTITY FINDING" |
