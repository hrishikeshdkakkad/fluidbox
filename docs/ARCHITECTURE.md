...
2. **Initialize.** The orchestrator prepares the workspace **control-plane-side**: the credentialed git fetch/copy happens before the agent exists. The sandbox will only ever see a bind-mounted copy at `/workspace` — the original repo is never touched, and the sandbox needs no network egress *for this step*. Whether it **has** egress is a separate, profile-specific question: the Kubernetes `zeroEgress` profile and Docker `Hardened` mode close it, and the Docker default (`HostDev`) deliberately does not — it also injects `host.docker.internal`, giving the sandbox the host's network position. See [Security boundaries](../README.md#security-boundaries).
...
5. **Decide.** Tool calls pass the single decision gate: budget → frozen capability availability → frozen-schema arguments → trust tier → policy → approvals. Two different strengths meet here, and the difference matters: a **brokered MCP** call cannot execute without a decision *structurally*, because the control plane is what executes it. An **in-sandbox** call (`Bash`, `Edit`, `Read`, sandbox stdio MCP) is executed by the sandbox, so the gate binds it only because the runner routes it there — on the Claude harness via a mandatory `PreToolUse` hook, proven by `scripts/gate-proof.sh`. Containment, not the gate, is the control that binds a harness or a pinned image that does not route. A `RequireApproval` verdict pauses the tool call until a human decides (idempotent by `(session_id, tool_call_id)`, safe across restarts) — or, in autonomous mode, is rewritten to the policy's fallback *inside* the evaluator, with both verdicts recorded.
