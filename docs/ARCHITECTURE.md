...
1. **Create.** All entry points — dashboard, CLI, API trigger, webhook, schedule — converge on one code path (`run_service::create_run`). It resolves the agent's current revision, intersects capability pins, and **freezes an immutable `RunSpec`**: model, system prompt, task, a full policy snapshot, capability schemas, budgets, and the invocation context. In-flight runs are governed by their snapshot; editing an agent or policy only affects future runs. This immutability is what makes the audit trail trustworthy.
...
- **Capabilities are exactly two tool classes, and the split is the security model.** *Sandbox* MCP servers are stdio subprocesses packaged in the runner image — credential-free by construction, contained by the container. *Brokered* MCP servers are called by the control plane behind the same decision gate. Attach ≠ allow: pins freeze what *exists* for a run; the gate decides every call.
