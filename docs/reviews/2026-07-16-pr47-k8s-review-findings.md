...
- [x] **H4. Any repo with a tracked symlink can never run on the K8s provider.** *(fixed: fix/k8s-symlink-archive — `unpack_archive` extracts symlinks whose target, resolved lexically against the link's own parent, stays inside the root; hardlinks and absolute/`..`-escaping targets still refused. Tests: `unpack_creates_intree_relative_symlink`, `unpack_rejects_escaping_symlink_targets`, `unpack_still_rejects_hardlink`, and end-to-end `symlink_survives_pack_unpack_and_appears_in_diff`.)*
...
- [ ] **L13.** Test-coverage gaps (beyond H1): ~~no symlink-repo fixture (H4)~~ *(added in batch 3)*, no cancel⇄result race test (H5), no truncated-exec-stream test (M2), no values→PodSpec chart test (M3).
