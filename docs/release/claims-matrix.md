...
| # | Claim | Class | Evidence |
|---|---|---|---|
| C1 | The eval Docker profile is for trying the run loop, not for exposing to a network. | **NOW TRUE, WITH A NAMED RESIDUAL** | The published admin-token default is gone (compose refuses without one) and the dashboard is loopback. The **API port is still published on all interfaces and cannot be loopback-bound** — sandboxes reach the control plane via `host.docker.internal`, the host gateway. The token is the control, not your network position. `deploy/compose-assertions.sh` (CI) guards all of it, including that the file still explains why. |
