...
| Linux | amd64 | **NOT VALIDATED for this candidate** | This is what CI runs (`ubuntu-latest`), so the Rust suite, the dashboard build, the acceptance suites, and the new compose/demo/gate-proof jobs will execute there on the next pipeline run — but that had not happened at the time of writing. |
