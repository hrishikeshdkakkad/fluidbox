...
10. How the dashboard edits governance without owning the policy language. — **SETTLED 2026-07-14: a per-tool permissions matrix over a separate `managed_overrides` column** (see `docs/plans/2026-07-14-governance-page-design.md` §7 for the four sub-decisions: matrix-not-YAML with conditional rules read-only; overrides as a UI-owned column so `yaml_source` stays git-owned and `policy-sync` needs no change; overrides precede `tools` in first-match-wins and are exact-name only; the matrix moves only the policy verdict — trust tier, budgets, and frozen-capability availability sit above it in `decide_tool_call`).
11. How policies are authored and audited once the dashboard owns them. — **SETTLED 2026-07-14: DB-native, append-only `policy_versions`, overrides collapsed into rules** (see `docs/plans/2026-07-14-db-native-policies-design.md` §8; **partially supersedes #10**). Files stay a boot seed (`seed_policy_if_absent` already never clobbers); `policy-sync.sh` retires because its force-push is the only thing that would overwrite UI-authored content, and YAML becomes an import/export format. Identity splits from content — `policies` keeps the stable id an `agent_revision` points at (unchanged FK), `policy_versions` is the append-only history; latest governs future runs, `RunSpec` still freezes. Version history + draft→publish replace git review and the PR beat (detective, not preventive — "require approval to publish" is the successor). `managed_overrides` (#10) collapses into head rules: the column existed **only** because the authored YAML was git-owned, and the fold must be verdict-preserving. Attribution is provenance (`seed|api|ui|import`), not a person — fluidbox has no user model.
