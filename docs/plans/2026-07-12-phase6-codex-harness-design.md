...
- **Step 4** (facade dialect + enforcement + metering + gate digest binding; reviewed at commit 955bc57 → REQUEST CHANGES, 3 MAJOR / 3 MINOR / 1 NIT; fixes landed in the follow-up commit):
  ...
  - MAJOR (concurrent same-`tool_call_id` handlers could return contradicting verdicts: the session-grant fast path returned allow while a racing handler had promoted the row to pending): **FIXED** — `record_intent_verdict` is now a compare-and-set returning who won; every gate path CAS-records its verdict and only the WINNER emits the ledger event; a loser adopts the durable outcome (`adopt_terminal_or_deny` for the deterministic paths, a shared `await_pending_decision` wait-join for the one path — session-grant-vs-promotion — that can lose to a pending row). One intent → one durable decision under races.
  ...
  - Pre-existing (NOT Phase-6 regressions), recorded: brokered execution is at-least-once under concurrent same-id calls (already documented in HANDOVER §4); concurrent human-approval waiters still each emit a (cosmetic, DB-is-truth) ApprovalDecided event.
