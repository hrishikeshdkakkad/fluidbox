...
- **Connector-catalog bulk import (schema + tooling)** — the catalog is now import-ready without importing a single row. A `provenance` column (migration 0009) makes every entry auditable and refreshable; curated seeds carry `{"source":"fluidbox"}` and can never be clobbered by an import. A new reference-only transport, `rest_action`, lets an imported open-connector provider (a REST-API Action, not a hosted MCP server) show up as a browsable Store card whose **Connect is refused** (`400`, "reference-only") until the deferred REST action executor lands; `GET /v1/catalog` now derives a `connectable` flag per entry so the dashboard can badge those cards. An offline dev tool, `just catalog-import` (`crates/fluidbox-catalog-import`), turns a **pinned** [open-connector](https://github.com/oomol-lab/open-connector) (Apache-2.0) checkout into a deterministic, append-only, sorted `INSERT … ON CONFLICT` migration of untrusted **community**-tier rows — running the SAME poison screen as capability registration over every imported string (offenders drop their whole provider), deduping slugs, and tagging each row with its upstream commit. The tool never runs at boot or request time and is not in the server crate graph; attribution is recorded in `NOTICE`. The actual breadth (the generated import migration) is a separate, legally-gated merge.
