...
" | { read a; read b; [ "$a" = "CANON" ] && lok "codex tool calls ledgered as CANONICAL names" || lno "non-canonical/absent tool.requested"; [ "$b" = "DECIDED" ] && lok "tool.decision events present (gated)" || lno "no tool.decision"; }
...
# no leaked secrets in the ledger
echo "$EV" | grep -qiE "sk-proj-|OPENAI_API_KEY=sk|$FLUIDBOX_SESSION_TOKEN" && lno "secret leaked in ledger!" || lok "no secrets in the ledger"
