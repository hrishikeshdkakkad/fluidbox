...
PYEOF
  local RUN_STATUS=$?
  if [ $RUN_STATUS -eq 130 ] || [ $RUN_STATUS -ge 128 ]; then
  ...
  [ $RUN_STATUS -le 3 ] || die "timeline watcher failed" "the demo stack was left up for inspection — remove it with: just demo-down"
...
print(f"  every tool call crossed the server-side gate: {len(dec)} decisions")
